...
def test_put_branding_rejects_svg(client):
    ...
    r = client.put("/api/v1/enterprise/branding", json={"company_name": "X", "logo_base64": svg_b64, "logo_mime": "image/svg+xml"}, headers={"Authorization": f"Bearer {token}"})
...
def test_put_branding_rejects_oversized_logo(client):
    ...
    r = client.put("/api/v1/enterprise/branding", json={"company_name": "X", "logo_base64": big, "logo_mime": "image/png"}, headers={"Authorization": f"Bearer {token}"})
    assert r.status_code == 422
