...
class ReportGenerator {
  ...
  async generateHTMLReport(frameworkId, brandingConfig = null) {
    ...
    const reportType = brandingConfig?.reportType || null; // 'type_1' | 'type_2'
    const periodStart = brandingConfig?.periodStart || null;
    const periodEnd = brandingConfig?.periodEnd || null;
    const isType2 = reportType === 'type_2';
    const engagementLabel = isType2
      ? 'Readiness — SOC 2 Type II (design & operating effectiveness)'
      : reportType === 'type_1'
        ? 'Readiness — SOC 2 Type I (design)'
        : 'Readiness (point-in-time)';
    const periodText = (isType2 && periodStart && periodEnd) ? `${periodStart} – ${periodEnd}` : null;
...
<div class="statement">
  ...
  evidence. A total of <strong>${evidence.length} evidence item(s)</strong> were considered.${isType2 && periodText ? ` This report supports a <strong>Type II</strong> readiness view: evidence is presented with collection dates across the assessment period (${escapeHtml(periodText)}) to indicate coverage of operating effectiveness. Independent testing of operating effectiveness is performed by the auditor, not by ComplianceGuard.` : ''}</p>
  ...
  <div class="scope-grid">
    ...
    <div class="row"><span class="k">${periodText ? 'Assessment period' : 'Assessment date (as of)'}</span><span class="v">${escapeHtml(periodText || dateText)}</span></div>
    ...
    <div class="row"><span class="k">Engagement type</span><span class="v">${escapeHtml(engagementLabel)}</span></div>
  </div>
