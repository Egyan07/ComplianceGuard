...
<head>
<meta charset="UTF-8">
<title>ComplianceGuard - ${framework.name} Compliance Report</title>
<style>
...
<div class="cover">
  ...
  <div class="report-title">${framework.name} Compliance Report</div>
...
<div class="content">
  ...
  ${framework.name} framework. The overall compliance score is
  <strong>${Math.round(overallScore)}%</strong> with a status of
  <span class="status ${status}">${status.replace(/_/g, ' ')}</span>.</p>
  ...
  <table>
    ...
    <tbody>
      ${Object.entries(findings.control_results).map(([id, ctrl]) => `<tr>
        ...
        <td>${ctrl.score || 0}%</td>
        <td><span class="status ${ctrl.status}">${(ctrl.status || '').replace(/_/g, ' ')}</span></td>
  ...
  ${findings.recommendations.map(rec => `
    <div class="recommendation ${rec.priority || 'medium'}">
      <div class="priority">${rec.priority || 'medium'} priority — ${rec.control_id}</div>
      <div>${rec.recommendation}</div>
      ${rec.evidence_needed && rec.evidence_needed.length > 0 ?
        `<div style="margin-top:6px;font-size:12px;color:#666"><strong>Evidence needed:</strong> ${rec.evidence_needed.join(', ')}</div>` : ''}
    </div>
  `).join('')}
  ` : ''}
  ...
  <div class="footer">
    ...
    Report generated on ${now.toISOString()} | Framework: ${framework.name} v${framework.version || '2017'}
  </div>
</div>
